...
- **リーダーボード** — モード別（チャレンジ／制覇 × 地形図／3D）にランキング（Supabase／Googleログイン）
- **サウンド演出** — 正解時・完了時の効果音
...
- **スコアDB**: Supabase（Googleログイン＋リーダーボードのみ、クイズデータ自体はDBを使わない）
...
│   ├── supabase.ts        # Googleログイン・スコア送信・ランキング
...
2. サイトを開く（**ログインなしで全部遊べます**。スコアを残したいときだけ Google でログイン）
