...
| モード②の3Dのテクスチャは**空中写真** | 地理院の全国最新写真（`seamlessphoto`）を貼り、その上に陰影起伏（hillshade）を薄く重ねる。**地形図（標準地図・淡色地図）は貼らない** | 地形図テクスチャには山名・三角点・注記が焼き込まれており、答えが画面に表示されてしまう。空中写真には注記が無く、森・岩・登山道の質感が出るので、陰影だけでは単調な面に見えていた近くの斜面まで読める |
...
- [x] **見回しても見下ろし角は変わらない**（＝首を振っただけで視線が上下しない）。視線の先に地面がある限り、操作した角度そのままを保つ
...
- [ ] **深い谷を見渡す向きだけ、見下ろし角が深くなる。** MapLibreは画面中央に「地面の1点」しか置けないので、視線の先1.2km以内に地面が無い向きでは、一番浅く見られる場所（＝地形の際）を狙うしかない。実測では大台ヶ原の大杉谷側だけで、5.5度→最大18度。頭の位置は動かないが、視線は下を向く。1.2kmを伸ばせば減らせるが、そのぶん地形とテクスチャが粗くなる（ズームが下がるため）ので、この妥協にしてある
