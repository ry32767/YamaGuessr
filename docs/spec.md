...
- [x] **マーカーは地面に接する。宙に浮かない。** カメラを動かす`jumpTo`には必ず`elevation`（地面の高さ・強調込み）を渡す。`centerClampedToGround`の自動追従に任せると中心標高が0に落ちたまま戻らないことがあり、マーカーが「海面にあるこの緯度経度」の位置に描かれて**標高に比例して浮く**（氷ノ山1296m地点で実測506px）。地点を移した直後は標高タイルがまだ無いので出題データの`elevation_m`で仮置きし、届いた時点で入れ直す
- [x] 標高の単位を取り違えない。`getElevationForLngLatZoom`も`jumpTo({elevation})`も**強調込み**の値を扱う。HUDに出す実標高は強調で割り戻す
...
#### 機能K：メールログイン・ニックネーム
> 匿名認証 →（2026-07-31）Googleログイン →（2026-08-02）**メールアドレス＋パスワード**。理由は下記。

- [x] **ログインは強制しない。** 未ログインでも出題・回答・スコア計算は最後まで動く（ニックネームは「ゲスト」、記録が残らないことだけを伝える）
- [x] ヘッダーの「ログイン」とホームの「メールアドレスで新規登録」から、同じダイアログを「ログイン／新規登録」タブ付きで開ける
- [x] 新規登録は**メールアドレス・パスワード・ニックネーム**の3項目で、登録と同時にサインインまで済む（確認メールなし運用）
- [x] 登録済みのメールアドレスとパスワードでログインすると、**別の端末・ブラウザでも同じプレイヤーとして復帰する**
- [x] 初回ログイン時のニックネームは**サーバ保存済み → 登録時に入力した値（`user_metadata`）→ メールアドレスの@より前**の順で決める（リーダーボードが「（名前なし）」で埋まらないように）
- [x] 確認メール待ちで登録が中断しても、**入力したニックネームを捨てない**（`players`を作れない経路なので`signUp`の`options.data`に預け、次のログインで拾い直す）
- [x] ヘッダーのニックネームボタンから変更でき、`players`が更新される。同じダイアログからログアウトできる
- [x] 入力エラー（形式・文字数）とサーバ側エラーを**日本語**でダイアログ内に出す。フォームは`novalidate`にして、ブラウザ既定の英語メッセージが先に出ないようにする
- [x] ダッシュボードで「Confirm email」がONに戻された場合も画面が黙り込まず、「確認メールを送りました」と案内する（`AuthOutcome.pending`）
- [x] **Supabaseに接続できない場合でも、ゲーム本体は最後まで動作する**（リーダーボード関連UIのみ無効）
- [x] 実キーでの疎通確認：リーダーボードの取得が200で返ることを確認（記録0件なら「まだ記録がありません」）
- [x] 実キーでのログイン確認：未登録の資格情報で「メールアドレスかパスワードが違います」が日本語で出て、送信ボタンが再度押せる状態に戻ることをブラウザで確認
- [ ] **登録〜スコア送信の実地確認**（ダッシュボードで「Confirm email」をOFFにしてから実施）

> **なぜGoogleログインをやめたか**：Google Cloud ConsoleでOAuthクライアントを作り、同意画面を通し、
> リダイレクトURIを登録する手間が、遊ぶまでの距離に対して重すぎた。**メール＋パスワードなら
> Supabase側の設定だけで完結し、外部プロバイダへのリダイレクトが無いのでURL Configurationも要らない**。
>
> **なぜ確認メールを送らないか**：Supabase無料枠の既定SMTPは「2通/時」でテスト用途と明記されており、
> 数人が同時に登録しようとすると詰まる。メールアドレスは**記録を引き継ぐためのIDとしてしか使わず**
> （通知も送らず、リーダーボードに出るのはニックネームだけ）、所有確認をしなくても実害が小さいため、
> 送信をゼロにする方を選んだ。カスタムSMTPを入れるならONに戻してよい（コードは両対応）。
>
> **なぜ匿名認証をやめたか**：端末ごとに別人になり、機種変更やブラウザを変えると記録が引き継げない。
> リーダーボードは「誰の記録か」が分かって初めて意味を持つので、ログインを1回だけ求める形にした。
> ただし**遊ぶだけならログイン不要**という性質は変えていない（DESIGN.md 不変条件7）。
...
- [x] ランキング取得は`leaderboard`ビューから行い、**`player_id`は要求しない**（ビューに無い列を要求して常に400になっていたのを修正。自分の行の判定は自分のスコアIDを別に引いて突き合わせる）
- [x] `leaderboard`ビューは`security_invoker = true`で作る（付けないと作成者権限で動きRLSを素通りする。Supabaseのlinterが`security_definer_view`をERRORで報告した）
- [x] **実プロジェクトへの適用確認**：`players`/`scores`/`leaderboard`が存在し、RLSポリシー5本とCHECK制約6本が`supabase/schema.sql`と一致することをMCP経由で照合。`scores`にupdate/deleteポリシーが無い（＝改ざん・削除不可）ことも確認
- [x] **リーダーボード画面の実挙動確認**：実キーでdevサーバを起動し、4組み合わせ（mode × view_mode）すべてでタブ切替→取得が成功し、0件時に「まだ記録がありません」が出る（取得失敗の表示にはならない）ことをブラウザで確認
- [ ] **スコア送信の実挙動確認**（insertはログインが要るため、「Confirm email」をOFFにして登録できるようにしてから実施）
...
- [x] 機能K：メールログイン・ニックネーム（障害耐性込み）→ 実装済み。**キー未設定でも1セッション完走できる**ことと、実キーでのエラー日本語化・375px表示を確認
- [x] 機能L：リーダーボード → 実装済み。実プロジェクトのスキーマ・RLS・CHECKを照合し、4組み合わせすべてで取得が通ることをブラウザで確認（送信は「Confirm email」OFF後）
