...
- [x] **その地点に立った一人称視点**で描画される（上空から見下ろす形ではない）。カメラは出題地点の地面＋目の高さ1.6mに固定される
- [x] **ドラッグで360度ぐるりと見回せる**（ストリートビューと同じで、視線だけが動き立ち位置は動かない）。矢印キーでも見回せる
- [x] 画面に「ここに立っています」「方位（○°／北向き等）」「標高」が常時表示され、自分がどこに立ってどちらを向いているか分かる
- [x] 足元に現在地の印が描かれ、見下ろすと立っている場所が分かる
- [x] 視点の高さは出題データの`elevation_m`から決める（地形タイルの読み込みを待たずに正しい視点になる）
...
#### 機能P：写真から出題地点を作る（`pipeline/import_photos.py`）
- [x] スマホ等で撮ったJPEG/TIFFの**EXIF撮影日時をGPXに突き合わせて位置を決める**（写真のGPSより、GPXの方が精度も一貫性も高いため）
- [x] EXIFの読み取りは外部ライブラリなしで行う（撮影日時・UTCオフセット・GPS・向きだけを読む）
- [x] EXIFにUTCオフセットが無い場合は`--tz`（既定 +09:00）を使う
- [x] GPXの時刻範囲から`--time-gap-limit-s`（既定300秒）以上外れた写真は採用せず、理由を出す（別の山行の写真が紛れても、違う場所の地点を作らない）
- [x] 画像は長辺1280pxのWebP・1枚200KB以下・**メタデータ全除去**で書き出し、ファイル名は`{point_id}.webp`
- [x] 各地点にルートの進行方位（`heading_route_deg`）と標高を付ける
- [x] HEIC/HEIFは読めないため、その旨を案内してスキップする（JPEGへの変換を促す）
- [x] 撮影時刻の順に並び、IDが連番になる
...
- [ ] 3Dビューは MapLibre の上限（pitch 85度）があるため、**真横より上は見上げられない**。空を見上げる必要は無い想定だが、山を見上げたい場面が出たら別の描画方法を検討する
- [ ] 3Dビューにズーム（遠くの尾根を引き寄せる）が無い。MapLibre に画角を変える公開APIが無いため見送った
