...
#### 機能K：Googleログイン・ニックネーム
> 匿名認証から**Googleログイン**に変更した（2026-07-31）。理由は下記。

- [x] **ログインは強制しない。** 未ログインでも出題・回答・スコア計算は最後まで動く（ニックネームは「ゲスト」、記録が残らないことだけを伝える）
- [x] ヘッダーの「Googleでログイン」から`signInWithOAuth`でGoogleへ遷移し、戻ってきたらセッションから同一プレイヤーとして復帰する
- [x] 初回ログイン時、**Googleの表示名を初期ニックネーム**にして`players`に登録する（リーダーボードが「（名前なし）」で埋まらないように）
- [x] ヘッダーのニックネームボタンから変更でき、`players`が更新される。同じダイアログからログアウトできる
- [x] ログインに失敗して戻ってきたとき（プロバイダ未設定など）、**URLに載ったエラーを画面に出す**（押しても何も起きないように見せない）
- [x] **Supabaseに接続できない場合でも、ゲーム本体は最後まで動作する**（リーダーボード関連UIのみ無効）
- [x] 実キーでの疎通確認：リーダーボードの取得が200で返ることを確認（記録0件なら「まだ記録がありません」）
- [ ] **Googleログインの実地確認**（Supabase側で Google プロバイダを有効化してから実施）
...
- [ ] **スコア送信・RLS/CHECK制約の実挙動確認**（Googleログインを有効化してから実施）
...
- [x] 機能K：Googleログイン・ニックネーム（障害耐性込み）→ 実装済み。**キー未設定でも1セッション完走できる**ことを確認
- [x] 機能L：リーダーボード → 実装済み。実キーで取得が通ることを確認（送信はログイン有効化後）
